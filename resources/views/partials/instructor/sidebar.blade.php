...
<aside class="portal-sidebar user-sidebar">
    ...
    <div class="portal-sidebar-profile user-head">
        <a href="{{ route('instructor.settings', ['locale' => $locale]) }}" class="portal-sidebar-profile__settings" title="الإعدادات" aria-label="الإعدادات">
        ...
        <div class="portal-sidebar-profile__info user-information">
            <h6 class="portal-sidebar-profile__name">{{ $staff?->name_ar ?? $user?->displayName() }}</h6>
            <p class="portal-sidebar-profile__email" dir="ltr" title="{{ $user?->email }}">{{ $user?->email }}</p>
            <span class="portal-sidebar-profile__role">مدرب</span>
        </div>
    </div>

    <nav class="portal-sidebar-nav user-body" aria-label="قائمة لوحة المدرب">
        <ul class="portal-sidebar-nav__list">
            @foreach (InstructorNavigation::items() as $item)
                <li>
                    <a href="{{ $item['route'] }}" @class(['portal-sidebar-nav__link', 'active' => InstructorNavigation::isActive($item['key'], $instructorActive)])>
