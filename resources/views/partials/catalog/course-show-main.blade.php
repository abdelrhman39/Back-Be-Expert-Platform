...
<div class="col-lg-8 course-show-tabs">
    @if ($tabs !== [])
        <div class="course-show-tabbar listing-tab listing-tab--with-scroll-controls" id="course-section-tabs">
            <button type="button" class="listing-tab-scroll-btn listing-tab-scroll-btn--inline-start" aria-label="{{ $isEn ? 'Scroll to previous tabs' : 'التمرير نحو بداية التبويبات' }}">
            ...
            <button type="button" class="listing-tab-scroll-btn listing-tab-scroll-btn--inline-end" aria-label="{{ $isEn ? 'Scroll to next tabs' : 'التمرير نحو نهاية التبويبات' }}">
    ...
    @else
        ...
        <div class="service-wrap">
            <p>{{ $isEn ? 'Details for this program are being prepared. Contact us for more information.' : 'تفاصيل هذه الدورة قيد الإعداد. تواصل معنا للاستفسار.' }}</p>
        </div>
    @endif
</div>
