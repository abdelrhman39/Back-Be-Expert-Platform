@php
    ...
    $iconHtml = $icons[$icon] ?? $icons['book'];
    $toneClass = ! empty($tone) ? ' admin-detail-field__icon--'.$tone : '';
@endphp

<div class="admin-detail-field">
    <span class="admin-detail-field__icon{{ $toneClass }}" aria-hidden="true">{!! $iconHtml !!}</span>
    <div class="admin-detail-field__body">
        <span class="admin-detail-field__label">{{ $label }}</span>
        <span class="admin-detail-field__value">{!! $value !!}</span>
    </div>
</div>
