@once
    @push('styles')
        <style>
            ...
            .commerce-btn-remove:hover:not(:disabled) {
                background: #dc3545;
                border-color: #dc3545;
                color: #fff;
            ...
            .commerce-btn-remove:disabled {
                opacity: .65;
                cursor: not-allowed;
            }

            .commerce-btn-remove__icon {
                display: inline-flex;
                width: 20px;
                height: 20px;
                flex-shrink: 0;
            ...
            .commerce-btn-remove__icon svg {
                width: 20px;
                height: 20px;
            }

            .commerce-btn-remove__icon svg path {
                transition: fill .2s ease, stroke .2s ease;
            }

            .commerce-btn-remove__icon svg .cart-stroke {
                stroke: currentColor;
            }

            .commerce-btn-remove__icon svg .cart-fill {
                fill: currentColor;
            }
        </style>
    @endpush
...
<button type="button"
    @if ($mode === 'cart')
        class="commerce-btn-remove makeCartlist_cart_page"
        data-course_id="{{ $item->course_id }}"
        data-type="{{ $item->delivery_type }}"
    @else
        class="commerce-btn-remove"
    @endif
