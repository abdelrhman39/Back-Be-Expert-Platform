...
@include('partials.instructor.shell-start', [
    'instructorActive' => 'notifications',
    'instructorTitle' => 'الإشعارات',
])
...
<div class="portal-dashboard portal-notifications-page">
    <div class="portal-orders-intro">
        <div class="portal-orders-intro__text">
            ...
            <p class="portal-orders-intro__desc">تحديثات الشعب والاختبارات والإعلانات الإدارية.</p>
        </div>
        @if ($user && $user->unreadNotifications()->exists())
            <button type="button" class="portal-btn-secondary" wire:click="markAllRead">تعيين الكل مقروءاً</button>
        @endif
    </div>
    ...
    @else
        <div class="portal-notifications-list">
            @foreach ($notifications as $notification)
                ...
                @endphp
                <article @class(['portal-notification-item', 'portal-notification-item--unread' => $isUnread]) wire:key="inst-n-{{ $notification->id }}">
                    ...
                    <div class="portal-notification-item__body">
                        ...
                        <div class="portal-notification-item__meta">
                            ...
                            @endif
                            @if ($isUnread)
                                <button type="button" class="portal-notification-item__read" wire:click="markRead('{{ $notification->id }}')">تعيين مقروءاً</button>
                            @endif
                        </div>
                    </div>
                </article>
